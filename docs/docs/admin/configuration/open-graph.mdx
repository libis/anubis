...
When fetching the target, Anubis forwards the visitor's original host as `X-Forwarded-Host`, mirroring what the normal proxy path sends. This lets backends that resolve their site from that header (e.g. WordPress multisite behind per-instance origin vhosts) serve the correct tags even when `--target-host` is set.
